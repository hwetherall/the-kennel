...
import { liveSquareId } from '../lib/game'
    ...
    <section className={`grid-card${projector ? ' grid-card--projector' : ''}${print ? ' grid-card--print' : ''}`}>
      ...
      <div className="grid-shell">
        ...
        <div className="squares-grid" role="grid" aria-label="100 square board">
          ...
          {grid.rowDigits.flatMap((rowDigit, rowIndex) => [
            <div
            ...
            ...Array.from({ length: 10 }, (_, colIndex) => {
              ...
              if (!square) return <div className="square" key={`empty-${rowIndex}-${colIndex}`} />
              ...
              return (
                <div
                  ...
                  key={square.id}
                  role="gridcell"
                  aria-label={`${rowDigit}-${grid.colDigits[colIndex]}: ${square.ownerLabel ?? 'Open square'}${isLive ? ', currently live' : ''}`}
                >
